...
/* ========= Canvas runtime with safe cleanup ========= */
export function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    // Type non-nullable : `tick` et `applySize` sont des déclarations de
    // fonction hoistées, et TS n'y propage pas le narrowing du `if` ci-dessus.
    ...
    const getDpr = () => {
      ...
      // Plancher abaisse a 0.35 : ce shader fait 80 iterations de
      // raymarching par pixel, le cout est proportionnel au nombre de
      // pixels rendus. Avec un plancher a 1 il etait impossible de le
      // rendre sous la resolution native, et la page tombait a 1 fps.
      return Math.max(0.35, Math.min(2, pixelRatio ?? sys));
    };
